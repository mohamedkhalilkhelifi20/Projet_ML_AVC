...
LABEL_MAPS = {
    ...
    'General health condition' : {1: 'Excellent', 2: 'Very Good', 3: 'Good',
                                  4: 'Fair', 5: 'Poor'},
    'depression'               : {1: 'Never', 2: 'Several days',
                                  3: 'More than half days'},
    'diabetes'                 : {0: 'No', 1: 'Yes'},
...
# Plages valides selon codebook NHANES
EXPECTED_VALUES = {
    'stroke'                   : {0, 1},
    'gender'                   : {1, 2},
    'age'                      : {1, 2, 3},
...
def check_data_quality(df):
    """
    Verifie valeurs manquantes et doublons.
    Definie une seule fois — appelee dans EDA, Preprocessing, Modeling.
    ...
    if n_dup == 0:
        ...
        print(f'  {n_dup} doublons detectes.')
...
def save_features(categorical_vars, numerical_vars):
    """
    Sauvegarde les listes de variables dans src/features.json.
    Appelee a la fin de l'EDA.

    Utilisation :
        from config import save_features
    ...
    print(f'features.json sauvegarde dans {FEATURES_JSON_PATH}')
...
def load_features():
    """
    Charge les listes de variables depuis src/features.json.
    Appelee dans Preprocessing, Feature Engineering et Modeling.
    ...
    if not os.path.exists(FEATURES_JSON_PATH):
        raise FileNotFoundError(
            ...
            f"Executer d'abord 1_EDA_data_v1.ipynb pour le generer."
        )

    with open(FEATURES_JSON_PATH) as f:
        features = json.load(f)

    print(f'features.json charge :')
